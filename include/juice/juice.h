...
//
// Compatibility shim: exposes the libjuice C API backed by stice.
// libdatachannel includes <juice/juice.h> and calls juice_* symbols;
// this header re-declares the libjuice API with the exact same types
// and the implementations (in juice_compat.cpp) forward to stice_*.
...
// TURN server transport protocol.
