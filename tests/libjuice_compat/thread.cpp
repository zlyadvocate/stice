...
int test_thread() {
	...
	config1.concurrency_mode = STICE_CONCURRENCY_MODE_THREAD;
	...
	config2.concurrency_mode = STICE_CONCURRENCY_MODE_THREAD;
	...
	// Check states
	stice_state_t state1 = stice_get_state(agent1);
	stice_state_t state2 = stice_get_state(agent2);
	bool success = (state1 == STICE_STATE_COMPLETED && state2 == STICE_STATE_COMPLETED);

	// Retrieve candidates
	char local[STICE_MAX_CANDIDATE_SDP_STRING_LEN];
	char remote[STICE_MAX_CANDIDATE_SDP_STRING_LEN];
	if (success &=
	    (stice_get_selected_candidates(agent1, local, STICE_MAX_CANDIDATE_SDP_STRING_LEN, remote,
	                                   STICE_MAX_CANDIDATE_SDP_STRING_LEN) == 0)) {
	...
	if (success &=
	    (stice_get_selected_candidates(agent2, local, STICE_MAX_CANDIDATE_SDP_STRING_LEN, remote,
	                                   STICE_MAX_CANDIDATE_SDP_STRING_LEN) == 0)) {
	...
	// Retrieve addresses
	char localAddr[STICE_MAX_ADDRESS_STRING_LEN];
	char remoteAddr[STICE_MAX_ADDRESS_STRING_LEN];
	if (success &= (stice_get_selected_addresses(agent1, localAddr, STICE_MAX_ADDRESS_STRING_LEN,
	                                             remoteAddr, STICE_MAX_ADDRESS_STRING_LEN) == 0)) {
		printf("Local address  1: %s\n", localAddr);
		printf("Remote address 1: %s\n", remoteAddr);
	}
	if (success &= (stice_get_selected_addresses(agent2, localAddr, STICE_MAX_ADDRESS_STRING_LEN,
	                                             remoteAddr, STICE_MAX_ADDRESS_STRING_LEN) == 0)) {
...
static void on_state_changed1(stice_agent_t *agent, stice_state_t state, void *user_ptr) {
	...
	if (state == STICE_STATE_CONNECTED) {
		// Agent 1: on connected, send a message
		const char *message = "Hello from 1";
		stice_send(agent, message, strlen(message));
	}
}
...
static void on_gathering_done1(stice_agent_t *agent, void *user_ptr) {
	...
	stice_set_remote_gathering_done(agent2); // optional
}

// Agent 2: on local candidates gathering done
static void on_gathering_done2(stice_agent_t *agent, void *user_ptr) {
	printf("Gathering done 2\n");
	stice_set_remote_gathering_done(agent1); // optional
}
